            ...
            <div class="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-blue-500/10 border border-blue-500/20 mb-4">
                <svg class="w-7 h-7 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                          d="M6.72 13.829c-.24.03-.48.062-.72.096m.72-.096a42.415 42.415 0 0110.56 0m-10.56 0L6.75 19.5l.001-.001
                             A21.998 21.998 0 0112 21c2.47 0 4.836-.413 7.002-1.172l.001.001L19.25 19.5l-.72-5.671
                             M3 10.5h18M6.75 8.25h10.5a2.25 2.25 0 002.25-2.25V6A2.25 2.25 0 0017.25 3.75H6.75
                             A2.25 2.25 0 004.5 6v.001a2.25 2.25 0 002.25 2.249z"/>
                </svg>
            </div>
            <h1 class="text-2xl font-bold text-white tracking-tight">Subir Archivo</h1>
            <p class="text-slate-400 text-sm mt-1">Formatos aceptados: PDF, JPG, PNG, DOCX, ODT, TXT, PS</p>
        </div>
        ...
        <form action="{{ route('trabajos.store') }}"
            ...
            <div>
                ...
                <select
                    ...
                    <option value="" disabled selected class="text-slate-500">Selecciona una impresora...</option>
                    @foreach($impresoras as $imp)
                        <option value="{{ $imp->id }}" {{ old('cups_trabajo_id') == $imp->id ? 'selected' : '' }}>
                            {{ $imp->nombre }}
                        </option>
                    @endforeach
                </select>
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <label class="relative cursor-pointer">
                        ...
                        <div class="flex items-center gap-3 px-4 py-3 rounded-xl border border-slate-600
                            ...
                            <span class="w-5 h-5 rounded-full border-2 border-current flex items-center justify-center shrink-0">
                                <span class="w-2 h-2 rounded-full bg-current opacity-0 peer-checked:opacity-100 transition"></span>
                            </span>
                    ...
                    <label class="relative cursor-pointer">
                        ...
                        <div class="flex items-center gap-3 px-4 py-3 rounded-xl border border-slate-600
                                    ...
                                    transition text-slate-400 peer-checked:text-white">
                            <span class="w-5 h-5 rounded-full border-2 border-current flex items-center justify-center shrink-0">
                                <span class="w-2 h-2 rounded-full bg-current opacity-0 transition"></span>
                            </span>
                ...
                <div id="dropZone"
                    ...
                    <div id="dropContent" class="pointer-events-none">
                        ...
                        <p class="text-slate-400 text-sm">
                            Arrastra tu archivo aquí o <span class="text-blue-400 font-medium">selecciona uno</span>
                        </p>
                        ...
                        <svg class="w-8 h-8 text-blue-400 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                  d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5
                                     a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Tipos MIME válidos (espejo del controlador PHP)
    const MIMES_VALIDOS = [
        'application/pdf',
        'image/jpeg',
        'image/png',
